...
def _find_testbench_fn(mod: object) -> Any | None:
    if not hasattr(mod, "tb"):
        return None
    tb_fn = getattr(mod, "tb")
    if not callable(tb_fn):
        raise SystemExit("tb must be a callable when present")
    if not bool(getattr(tb_fn, "__pycircuit_testbench__", False)):
        raise SystemExit("tb(...) must be decorated with `@testbench` when present")
    return tb_fn
...
def _collect_testbench_payload(
    ...
    *,
    ...
    program_export = payload_obj.as_dict()
    program_export["tb_name"] = str(tb_name)
    if trace_plan is not None:
        program_export["trace_plan"] = trace_plan.as_dict()
    payload = dict(program_export)
...
def _iface_port_rows(iface: _TopIface) -> list[dict[str, Any]]:
    ...
    for raw, safe, ty in zip(iface.in_raw, iface.in_names, iface.in_tys):
        rows.append(
            {
                "direction": "input",
                "raw_name": raw,
                "safe_name": safe,
                "ty": ty,
                "width_bits": width_of(ty),
            }
        )
    for raw, safe, ty in zip(iface.out_raw, iface.out_names, iface.out_tys):
        rows.append(
            {
                "direction": "output",
                "raw_name": raw,
...
def _render_cmodel_bridge_hpp(iface: _TopIface) -> str:
    top = iface.sym
    bridge = f"{top}CModel"
    lines: list[str] = []
    lines.append("// Generated by pycircuit: external C++ / TLM bridge scaffold.\n")
    ...
    lines.append("struct has_comb<T, std::void_t<decltype(std::declval<T &>().comb())>> : std::true_type {};\n\n")
    lines.append("template <typename T, typename = void>\n")
    lines.append("struct has_eval : std::false_type {};\n\n")
    lines.append("template <typename T>\n")
    lines.append("struct has_eval<T, std::void_t<decltype(std::declval<T &>().eval())>> : std::true_type {};\n\n")
    lines.append("template <typename T, typename = void>\n")
    lines.append("struct has_transfer : std::false_type {};\n\n")
    lines.append("template <typename T>\n")
    lines.append("struct has_transfer<T, std::void_t<decltype(std::declval<T &>().transfer())>> : std::true_type {};\n\n")
    lines.append("template <typename T>\n")
    lines.append("inline void maybe_comb(T &dut) {\n")
    lines.append("  if constexpr (has_comb<T>::value) {\n")
    lines.append("    dut.comb();\n")
    lines.append("  } else if constexpr (has_eval<T>::value) {\n")
    lines.append("    dut.eval();\n")
    lines.append("  }\n")
    lines.append("}\n\n")
    lines.append("template <typename T>\n")
    lines.append("inline void maybe_transfer(T &dut) {\n")
    lines.append("  if constexpr (has_transfer<T>::value)\n")
...
def _render_cmodel_entry_cpp(
    iface: _TopIface,
    *,
    bridge_header_name: str,
    tb_program_rel: str | None,
) -> str:
    top = iface.sym
    bridge = f"{top}CModel"
    lines: list[str] = []
    lines.append("// Generated by pycircuit: replace this scaffold with a hand-written C++ TB or TLM.\n")
    lines.append("#include <iostream>\n\n")
    ...
    if tb_program_rel is not None:
        lines.append(f"  std::cout << \"Stimulus bridge: {tb_program_rel}\\\\n\";\n")
...
def _render_cmodel_readme(
    iface: _TopIface,
    *,
    bridge_header_name: str,
    entry_cpp_name: str,
    tb_program_name: str | None,
) -> str:
    lines: list[str] = []
    lines.append(f"# {iface.sym} C-model Bridge\n\n")
    ...
    if tb_program_name is not None:
        lines.append(f"- `{tb_program_name}`: exported pyCircuit testbench program JSON for external replay\n")
    lines.append("\n")
    lines.append("Ports:\n")
    for row in _iface_port_rows(iface):
        lines.append(
            f"- `{row['direction']}` `{row['raw_name']}` as `dut.{row['safe_name']}` (`{row['ty']}`, {row['width_bits']} bits)\n"
        )
    lines.append("\n")
    lines.append("Typical workflow:\n")
    lines.append("1. Build the DUT with `python3 -m pycircuit.cli build <design.py> --target cpp --out-dir <out>`.\n")
    lines.append(f"2. Start from `{entry_cpp_name}` and replace the placeholder logic with your custom driver.\n")
    if tb_program_name is not None:
        lines.append(f"3. If you want pyCircuit-authored stimulus, replay `{tb_program_name}` in your external model.\n")
...
def _emit_cmodel_scaffold(
    *,
    out_dir: Path,
    iface: _TopIface,
    tb_program: Mapping[str, Any] | None = None,
) -> dict[str, Path]:
    cmodel_dir = out_dir / "cmodel"
    cmodel_dir.mkdir(parents=True, exist_ok=True)

    bridge_hpp = cmodel_dir / f"{iface.sym}_cmodel.hpp"
    entry_cpp = cmodel_dir / f"{iface.sym}_main.cpp"
    tb_program_path: Path | None = None
    if tb_program is not None:
        tb_program_path = cmodel_dir / "tb_program.json"
        _save_json(tb_program_path, dict(tb_program))

    _write_text_atomic(bridge_hpp, _render_cmodel_bridge_hpp(iface))
    _write_text_atomic(
        entry_cpp,
        _render_cmodel_entry_cpp(
            iface,
            bridge_header_name=bridge_hpp.name,
            tb_program_rel=(tb_program_path.name if tb_program_path is not None else None),
        ),
    )
    readme = cmodel_dir / "README.md"
    _write_text_atomic(
        readme,
        _render_cmodel_readme(
            iface,
            bridge_header_name=bridge_hpp.name,
            entry_cpp_name=entry_cpp.name,
            tb_program_name=(tb_program_path.name if tb_program_path is not None else None),
        ),
    )

    out = {
        "dir": cmodel_dir,
        "bridge_hpp": bridge_hpp,
        "entry_cpp": entry_cpp,
        "readme": readme,
    }
    if tb_program_path is not None:
        out["tb_program"] = tb_program_path
    return out
...
def _cmd_build(args: argparse.Namespace) -> int:
    ...
    tb_cpp_out = (out_dir / "tb" / f"{tb_name}.cpp") if tb_name is not None else None
    tb_sv_out = (out_dir / "tb" / f"{tb_name}.sv") if tb_name is not None else None
    ...
    if do_cpp and tb_name is not None and tb_pyc_path is not None and tb_cpp_out is not None:
        tb_key = f"tb:{tb_name}"
    ...
    if do_v and tb_name is not None and tb_pyc_path is not None and tb_sv_out is not None:
        tb_key = f"tb:{tb_name}"
    ...
    if do_cpp:
        ...
        cmodel_paths = _emit_cmodel_scaffold(out_dir=out_dir, iface=iface, tb_program=tb_program_export)
        cmodel_include_dirs = list(include_dirs)
        cmodel_dir_str = str(cmodel_paths["dir"])
        if cmodel_dir_str not in cmodel_include_dirs:
            cmodel_include_dirs.append(cmodel_dir_str)
        cmodel_headers = [*cpp_headers, cmodel_paths["bridge_hpp"]]
        cmodel_manifest = {
            "version": 4,
            "target_name": iface.sym,
            "entry_kind": "cmodel",
            "entry_cpp": str(cmodel_paths["entry_cpp"]),
            "sources": [str(p) for p in cpp_sources],
            "headers": [str(p) for p in cmodel_headers],
            "include_dirs": cmodel_include_dirs,
            "runtime": runtime,
            "cxx_standard": "c++17",
            "profile": str(args.profile),
            "executable_name": "pyc_cmodel",
            "dut_type": f"pyc::gen::{iface.sym}",
            "top_header": f"{iface.sym}.hpp",
            "ports": _iface_port_rows(iface),
        }
        cmodel_manifest_path = out_dir / "cmodel_project_manifest.json"
        _save_json(cmodel_manifest_path, cmodel_manifest)
        cmodel_section = {
            "manifest": str(cmodel_manifest_path.relative_to(out_dir)),
            "entry_cpp": str(cmodel_paths["entry_cpp"].relative_to(out_dir)),
            "bridge_hpp": str(cmodel_paths["bridge_hpp"].relative_to(out_dir)),
            "readme": str(cmodel_paths["readme"].relative_to(out_dir)),
        }
        if "tb_program" in cmodel_paths:
            cmodel_section["tb_program"] = str(cmodel_paths["tb_program"].relative_to(out_dir))
        manifest["cmodel"] = cmodel_section

        cpp_manifest = out_dir / "cpp_project_manifest.json"
        build_manifest = dict(cmodel_manifest)
        if tb_name is not None and tb_cpp_out is not None:
            if not tb_cpp_out.is_file():
                raise SystemExit(f"build(cpp): missing generated TB C++ source: {tb_cpp_out}")
            build_manifest = {
                "version": 4,
                "target_name": iface.sym,
                "entry_kind": "testbench",
                "entry_cpp": str(tb_cpp_out),
        ...
        manifest["cpp_executable"] = str(cmake_build / str(build_manifest.get("executable_name", "pyc_tb")))

    if do_v:
        if tb_name is None or tb_sv_out is None:
            if bool(args.run_verilator):
                raise SystemExit("build(verilator): --run-verilator requires `@testbench def tb(t: Tb): ...`")
        elif not tb_sv_out.is_file():
...
def main(argv: list[str] | None = None) -> int:
    ...
    build.add_argument(
        "python_file",
        help="Python source defining `@module build(...)`; `@testbench tb(...)` is optional",
    )
