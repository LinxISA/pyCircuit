...
def _is_timed_domain_build(build: Any) -> bool:
    """Return whether ``build`` uses the timed-domain entrypoint shape."""
...
def _collect_jit_params(build: Any, *, overrides: list[str]) -> dict[str, object]:
    if not callable(build):
        raise SystemExit(
            "build must be a callable @module entrypoint: `def build(m: Circuit, ...)`"
        )
    ...
    if not params:
        raise SystemExit(
            "build must use JIT entry semantics: `@module def build(m: Circuit, ...)`"
        )
...
def _compile_to_design(
    build: Any, *, top_name: str, jit_params: dict[str, object]
) -> Design:
    ...
    if _is_timed_domain_build(build):
        ...
        if not isinstance(compiled, Design):
            raise SystemExit(
                "internal error: expected Design from timed-domain compile"
            )
        return compiled
    try:
...
def _render_tb_cpp_sidecar(
    ...
    *,
    ...
    if trace_plan and trace_plan.enabled_signals:
        raise SystemExit(
            "sidecar C++ TB currently does not support trace-config binary traces; use --tb-schedule-mode=inline"
        )
    ...
    def words_array_literal(words: list[int], max_words: int) -> str:
        ...
        return (
            "{{"
            + ", ".join(
                f"0x{int(w) & ((1 << 64) - 1):x}ull" for w in padded[:max_words]
            )
            + "}}"
        )
    ...
    def register_port(raw: str, direction: str, ty: str) -> None:
        ...
        w = (
            1
            if resolved_ty in {"!pyc.clock", "!pyc.reset"}
            else _as_int_width(resolved_ty)
        )
        pid = get_port_id(sn)
        meta: dict[str, Any] = {
            "id": int(pid),
    ...
    for raw, ty in zip(iface.in_raw, iface.in_tys, strict=True):
        register_port(str(raw), "input", str(ty))
    for raw, ty in zip(iface.out_raw, iface.out_tys, strict=True):
    ...
    for d in t.drives:
        ...
        w = _as_int_width(ty)
        drive_events.append(
            (int(d.at), get_port_id(sn), sn, w, value_words(d.value, w))
        )
    ...
    for p in getattr(t, "prints", []):
        ...
        for raw in p.ports:
            ...
            if w > 64:
                raise SystemExit(
                    f"print() for i{w} not supported in sidecar C++ TB generator"
                )
    ...
    if t.random_streams:
        ...
        for r in t.random_streams:
            ...
            if w > 64:
                raise SystemExit(
                    f"random() for i{w} not supported in sidecar C++ TB generator"
                )
    ...
    drive_ports = sorted(
        {(pid, sn, w) for _cyc, pid, sn, w, _words in drive_events}, key=lambda x: x[0]
    )
    drive_slot_by_pid = {
        int(pid): slot for slot, (pid, _sn, _w) in enumerate(drive_ports)
    }
    ...
    expect_ports = sorted(
        {
            (pid, sn, w)
            for _cyc, pid, sn, w, _words, _msg in [
                *pre_expect_events,
                *post_expect_events,
            ]
        },
    ...
    def emit_event_array(
        name: str,
        rows: (
            list[tuple[int, int, str, int, list[int]]]
            | list[tuple[int, int, str, int, list[int], str]]
        ),
    ) -> list[str]:
        out: list[str] = []
        if not rows:
    ...
    drive_port_ids_literal = (
        "{" + ", ".join(f"{int(pid)}u" for pid, _sn, _w in drive_ports) + "}"
    )
    ...
    lines.append(
        f"static constexpr std::uint32_t kMaxEventWords = {int(max_words)}u;\n\n"
    )
    lines.append(
        f"static constexpr std::uint32_t kDrivePortCount = {len(drive_ports)}u;\n"
    )
    lines.append(
        f"static constexpr std::array<std::uint32_t, kDrivePortCount> kDrivePortIds = {drive_port_ids_literal};\n"
    )
    lines.append("using SidecarEvent = pyc::cpp::SidecarEvent<kMaxEventWords>;\n")
    lines.append(
        "using SidecarDriveFrame = pyc::cpp::SidecarDriveFrame<kMaxEventWords, kDrivePortCount>;\n\n"
    )
    ...
    lines.append(
        "    return ((frame.port_mask[slot / 64u] >> (slot % 64u)) & 1ull) != 0ull;\n"
    )
    lines.append("  };\n")
    for slot, (_pid, sn, w) in enumerate(drive_ports):
        lines.append(
            f"  if (hasDrive({int(slot)}u)) dut.{sn} = {wire_from_frame_expr(w, slot)};\n"
        )
    lines.append("}\n\n")

    lines.append("template <typename Dut>\n")
    lines.append(
        "void applyPeriodicDrive(Dut &dut, const pyc::cpp::SidecarPeriodicDrive &pattern, std::uint64_t cyc) {\n"
    )
    lines.append(
        "  const auto &words = pattern.activeAt(cyc) ? pattern.active_words : pattern.default_words;\n"
    )
    lines.append("  switch (pattern.port_id) {\n")
    for pid, sn, w in drive_ports:
        lines.append(f"  case {int(pid)}u:\n")
        lines.append(
            f"    dut.{sn} = pyc::cpp::Wire<{int(w)}>({{{', '.join(f'words[{i}]' for i in range(nwords(w)))}}});\n"
        )
        lines.append("    return;\n")
    lines.append("  default:\n")
    lines.append(
        '    std::cerr << "ERROR: invalid periodic drive port_id=" << pattern.port_id << " at cycle=" << cyc << "\\n";\n'
    )
    ...
    lines.append("template <typename Dut>\n")
    lines.append(
        "bool checkExpect(Dut &dut, const SidecarEvent &ev, const char *phase) {\n"
    )
    ...
    lines.append("  default:\n")
    lines.append(
        '    std::cerr << "ERROR(" << phase << "): invalid expect port_id=" << ev.port_id << " at cycle=" << ev.cycle << "\\n";\n'
    )
    lines.append("    return false;\n")
    lines.append("  }\n")
    lines.append("}\n\n")
    ...
    lines.append(
        "  pyc::cpp::SidecarRunnerSchedule<kMaxEventWords, kDrivePortCount> schedule;\n"
    )
    lines.append("  pyc::cpp::SidecarSchedule sidecar_schedule;\n")
    lines.append("  std::string sidecar_error;\n")
    lines.append(
        "  if (!pyc::cpp::loadSidecarSchedule(schedule_path, &sidecar_schedule, &sidecar_error)) {\n"
    )
    lines.append(
        '    std::cerr << "ERROR: failed to load sidecar schedule: " << sidecar_error << "\\n";\n'
    )
    lines.append("    return 1;\n")
    lines.append("  }\n")
    lines.append(
        "  if (!pyc::cpp::convertSidecarToRunnerSchedule(sidecar_schedule, kDrivePortIds, &schedule, &sidecar_error)) {\n"
    )
    lines.append(
        '    std::cerr << "ERROR: failed to convert sidecar schedule: " << sidecar_error << "\\n";\n'
    )
    lines.append("    return 1;\n")
    lines.append("  }\n\n")
    if rand_specs:
    ...
    lines.append(
        "    std::filesystem::path out_dir = std::filesystem::path(trace_dir_env);\n"
    )
    ...
    lines.append(
        "      if (cyc >= pattern.start_cycle && cyc < pattern.end_cycle) applyPeriodicDrive(dut, pattern, cyc);\n"
    )
    lines.append("    }\n")
    lines.append(
        "    while (drive_frame_idx < schedule.drive_frames.size() && schedule.drive_frames[drive_frame_idx].cycle == cyc) {\n"
    )
    lines.append(
        "      applyDriveFrame(dut, schedule.drive_frames[drive_frame_idx]);\n"
    )
    lines.append("      ++drive_frame_idx;\n")
    lines.append("    }\n")
    lines.append(
        "    if (pre_expect_idx < schedule.pre_expect_events.size() && schedule.pre_expect_events[pre_expect_idx].cycle == cyc) {\n"
    )
    lines.append("      pyc::cpp::detail::maybe_comb(dut);\n")
    lines.append("    }\n")
    lines.append(
        "    while (pre_expect_idx < schedule.pre_expect_events.size() && schedule.pre_expect_events[pre_expect_idx].cycle == cyc) {\n"
    )
    lines.append(
        '      if (!checkExpect(dut, schedule.pre_expect_events[pre_expect_idx], "pre")) return 1;\n'
    )
    ...
    lines.append(
        "    while (post_expect_idx < schedule.post_expect_events.size() && schedule.post_expect_events[post_expect_idx].cycle == cyc) {\n"
    )
    lines.append(
        '      if (!checkExpect(dut, schedule.post_expect_events[post_expect_idx], "post")) return 1;\n'
    )
    ...
    lines.append("  if (!ok) {\n")
    lines.append(
        '    std::cerr << "TIMEOUT: finish cycle not reached within " << timeout_cycles << " cycles\\n";\n'
    )
    lines.append("    return 1;\n")
    lines.append("  }\n")
    ...
    *,
    ...
    if rand_specs:
        ...
        for sn, w, ty, _seed, st, ev in rand_specs:
            ...
            lines.extend(
                drive_expr_lines(sn, f"(0x{mask:x}ull & rng_{sn})", ty, indent="      ")
            )
            lines.append("    }\n")
        lines.append("\n")
    ...
    if expects_pre_by:
        ...
        for cyc in sorted(expects_pre_by.keys()):
            ...
            for sn, val, msg, ty in expects_pre_by[cyc]:
                ...
                lines.extend(
                    expect_lines(sn, val, m, ty, indent="      ", phase="(pre)")
                )
            lines.append("      break; }\n")
        lines.append("    default: break;\n")
        lines.append("    }\n")
                ...
                for fmt, ports in prints_at[cyc]:
                    ...
                    for raw, sn, w in ports:
                        ...
                        if w == 1:
                            ...
                            lines.append(
                                f' << {raw_lit} << "0x" << std::hex << {expr} << std::dec'
                            )
                    lines.append(' << "\\n";\n')
                lines.append("      break; }\n")
            lines.append("    default: break;\n")
...
def _render_tb_sv(
    ...
    def sv_drive_const_lines(
        name: str, value: int | bool, ty: str, *, indent: str
    ) -> list[str]:
        info = _port_info(ty)
    ...
    def sv_drive_expr_lines(name: str, expr: str, ty: str, *, indent: str) -> list[str]:
        ...
        if info.total_width > 64:
            raise SystemExit(
                f"random() for i{info.total_width} not supported in SV TB generator (prototype limitation)"
            )
        hi = 63 if info.total_width >= 64 else (info.total_width - 1)
        return [f"{indent}{name} = {expr}[{hi}:0];\n"]

    def sv_expect_lines(
        name: str, value: int | bool, msg: str, ty: str, *, indent: str, phase: str
    ) -> list[str]:
        info = _port_info(ty)
        prefix = "PRE: " if phase == "pre" else ""
        return [
            f'{indent}if ({name} !== {sv_lit(info.total_width, value)}) $fatal(1, "{prefix}{msg}");\n'
        ]
    ...
    if expects_pre_by:
        ...
        for cyc in sorted(expects_pre_by.keys()):
            ...
            for sn, val, msg, ty in expects_pre_by[cyc]:
                m = msg if msg is not None else f"{sn} mismatch"
                lines.extend(
                    sv_expect_lines(sn, val, m, ty, indent="          ", phase="pre")
                )
            lines.append("        end\n")
        lines.append("        default: begin end\n")
        lines.append("      endcase\n")
    ...
    if expects_post_by:
        ...
        for cyc in sorted(expects_post_by.keys()):
            ...
            for sn, val, msg, ty in expects_post_by[cyc]:
                m = msg if msg is not None else f"{sn} mismatch"
                lines.extend(
                    sv_expect_lines(sn, val, m, ty, indent="          ", phase="post")
                )
            lines.append("        end\n")
        lines.append("        default: begin end\n")
        lines.append("      endcase\n")
        ...
        for cyc in sorted(prints_at.keys()):
            ...
            for fmt, ports in prints_at[cyc]:
                ...
                if ports:
                    ...
                    args = ", ".join(
                        ["cyc", *(sv_packed_expr(sn, ty) for _raw, sn, ty in ports)]
                    )
                    lines.append(
...
def _cmd_sidecar_inspect(args: argparse.Namespace) -> int:
    ...
    return (
        1 if bool(report.get("errors")) and bool(getattr(args, "strict", False)) else 0
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    sidecar = sub.add_parser(
        "sidecar", help="Inspect and verify sidecar schedule files."
    )
    sidecar_sub = sidecar.add_subparsers(dest="sidecar_cmd", required=True)

    sidecar_inspect = sidecar_sub.add_parser(
        "inspect", help="Print a human-readable sidecar section summary."
    )
    sidecar_inspect.add_argument("file", help="sidecar file path")
    sidecar_inspect.add_argument(
        "--strict",
        action="store_true",
        help="Return non-zero if framework-level errors exist.",
    )
    sidecar_inspect.set_defaults(fn=_cmd_sidecar_inspect)

    sidecar_verify = sidecar_sub.add_parser(
        "verify", help="Verify sidecar container and section-directory consistency."
    )
