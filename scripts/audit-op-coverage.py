...
def extract_ops(td_path: Path) -> dict[str, str]:
    ...
    for m in re.finditer(r'def\s+(ACIR_\w+Op|ACSim_\w+Op)\s*:\s*\w+_Op<"([^"]+)"', text):
...
def main():
    acir_ops = extract_ops(REPO / "include/acir/Dialect/ACIR/ACIROps.td")
    acsim_ops = extract_ops(REPO / "include/acir/Dialect/ACSim/ACSimOps.td")
    ...
    if missing_any == 0:
        ...
        print(f"FAIL: {missing_any} operations have incomplete coverage "
