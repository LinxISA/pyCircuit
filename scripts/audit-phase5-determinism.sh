#!/usr/bin/env bash
# Repeat the canonical Phase 5 NPU pipeline under unrelated roots and require
# byte-identical trace, result, statistics, event, replay, and Perfetto output.
set -euo pipefail

ROOT=$(cd "$(dirname "$0")/.." && pwd -P)
BUILD_DIR=${BUILD_DIR:-"$ROOT/build/dev-llvm22"}
RUNS=${RUNS:-3}
...
export PYTHONPATH="$ROOT/src:$BUILD_DIR/python${PYTHONPATH:+:$PYTHONPATH}"

EXAMPLE="$ROOT/examples/phase5/npu"
...
TEMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/agentic-circuit-phase5.XXXXXX")
...
"$GFSIM_TESTS" \
  --gtest_filter='ShowcaseTest.LegalWorkOrdersHaveByteIdenticalCommittedResults:NpuDependencyTrackerTest.WorkPermutationPreservesQueuesStatisticsAndObservations:Phase5NpuTraceSourceTest.RejectsUnsupportedOpcodeWithoutCommit'
...
for ((iteration = 1; iteration <= RUNS; ++iteration)); do
  ...
  "$PYTHON_BIN" "$ROOT/tools/import-davincioo-pto-trace.py" \
    "$workspace/traces/davincioo.jsonl" "$generated_trace" \
    --source-program examples/phase5/npu
  ...
  "$PYTHON_BIN" "$ROOT/tools/pack-perfetto-trace.py" \
...
echo "phase5 determinism audit: PASS ($RUNS runs)"
