...
class ConfigAndJitTest(unittest.TestCase):
    ...
    def test_checked_in_specialization_materializes_acir_and_cpp(self) -> None:
        path = REPOSITORY / "examples" / "v03" / "davincioo_jit.py"
        spec = importlib.util.spec_from_file_location("ac_v03_davincioo", path)
        if spec is None or spec.loader is None:
            raise RuntimeError("cannot load v0.3 specialization example")
    ...
    def test_cpp_specialization_cache_misses_then_hits(self) -> None:
        path = REPOSITORY / "examples" / "v03" / "davincioo_jit.py"
        spec = importlib.util.spec_from_file_location("ac_v03_jit_cache", path)
        if spec is None or spec.loader is None:
            raise RuntimeError("cannot load v0.3 specialization example")
        module = importlib.util.module_from_spec(spec)
        sys.modules[spec.name] = module
    ...
    def test_pyc_cpp_verilog_specialization_cache_when_toolchain_is_available(
        ...
        path = REPOSITORY / "examples" / "v03" / "davincioo_jit.py"
        spec = importlib.util.spec_from_file_location("ac_v03_pyc_cache", path)
        if spec is None or spec.loader is None:
            raise RuntimeError("cannot load v0.3 specialization example")
        module = importlib.util.module_from_spec(spec)
        sys.modules[spec.name] = module
