...
import sys
from pathlib import Path
from tests.python_frontend.test_lower_acir import elaborate
...
def frontend_test_ledger() -> dict[str, CoverageRow]:
    public_import = (
        "tests.python_frontend.test_public_api."
        "PublicApiTest.test_exact_public_inventory_is_importable"
    )
    definition_positive = (
        "tests.python_frontend.test_definitions."
    ...
    collection_negative = (
        "tests.python_frontend.test_scopes_collections."
    ...
    return {
        ...
        "process": CoverageRow(
            (
                "tests.python_frontend.test_process."
        ...
        "queue": CoverageRow(
            (
                "tests.python_frontend.test_resources."
...
class FrontendDeterminismTest(unittest.TestCase):
    ...
    def test_every_public_name_has_positive_and_negative_coverage(self) -> None:
        ...
        self.assertTrue(
            all(row.positive and row.negative for row in ledger.values())
        )
