...
def _committed_event(
    ...
    if not reserved.issubset(
        _COMMITTED_ARGUMENTS | _OPTIONAL_COMMITTED_ARGUMENTS
    ):
    ...
    delta = _safe_uint(
        arguments["gfsim_epoch_delta"], line=line, name="epoch delta"
    )
    owner = _safe_uint(
        arguments["gfsim_object_id"], line=line, name="object id"
    )
    ...
    if delta >= _MAX_DELTA or owner != thread or time > (
        _SAFE_INTEGER_MAX - delta
    ) // _MAX_DELTA or timestamp != time * _MAX_DELTA + delta:
        _fail(
            "ACPERFETTO-SCHEMA",
