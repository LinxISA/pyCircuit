...
class _Validator:
    ...
    def _symbolic_operand(self, node: ast.AST) -> ast.Name | None:
        ...
        return min(matches, key=lambda item: (item.lineno, item.col_offset), default=None)
        ...
        for candidate in ast.walk(node):
            if isinstance(candidate, self._forbidden_expressions):
                ...
                if isinstance(candidate.func, ast.Name):
                    ...
                    self._error(
                        "ACPY-SYNTAX-001", "dynamic call target is not portable", candidate
                    )
    ...
    def validate_statement(self, statement: ast.stmt) -> None:
        ...
        if isinstance(statement, ast.With):
            ...
            if not valid_scope:
                self._error(
                    "ACPY-SYNTAX-001", "only with scope(static_name) is supported", statement
                )
            for item in statement.items:
