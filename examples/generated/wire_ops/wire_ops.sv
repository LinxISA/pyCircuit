`include "pyc_add.sv"
`include "pyc_mux.sv"
`include "pyc_and.sv"
`include "pyc_or.sv"
`include "pyc_xor.sv"
`include "pyc_not.sv"
`include "pyc_reg.sv"
`include "pyc_fifo.sv"

`include "pyc_byte_mem.sv"

// Generated by pyc-compile (pyCircuit)
// Module: WireOps

module WireOps (
  input logic sys_clk,
  input logic sys_rst,
  input logic [7:0] a,
  input logic [7:0] b,
  input logic sel,
  output logic [7:0] y
);

logic [7:0] pyc_constant_1; // op=pyc.constant
logic pyc_constant_2; // op=pyc.constant
logic [7:0] a__wire_ops__L9; // pyc.name="a__wire_ops__L9"
logic [7:0] b__wire_ops__L10; // pyc.name="b__wire_ops__L10"
logic sel__wire_ops__L11; // pyc.name="sel__wire_ops__L11"
logic [7:0] pyc_xor_3; // op=pyc.xor
logic [7:0] COMB__y__wire_ops__L14; // pyc.name="COMB__y__wire_ops__L14"
logic [7:0] pyc_and_4; // op=pyc.and
logic [7:0] COMB__y__wire_ops__L16; // pyc.name="COMB__y__wire_ops__L16"
logic [7:0] pyc_mux_5; // op=pyc.mux
logic [7:0] COMB__y__wire_ops__L15; // pyc.name="COMB__y__wire_ops__L15"
logic [7:0] pyc_comb_6; // op=pyc.comb
logic pyc_comb_7; // op=pyc.comb
logic [7:0] pyc_comb_8; // op=pyc.comb
logic [7:0] y_reg__next; // pyc.name="y_reg__next"
logic [7:0] pyc_reg_9; // op=pyc.reg
logic [7:0] y_reg; // pyc.name="y_reg"
logic [7:0] r__wire_ops__L18; // pyc.name="r__wire_ops__L18"
logic [7:0] pyc_comb_10; // op=pyc.comb

assign pyc_constant_1 = 8'd0;
assign pyc_constant_2 = 1'd1;
assign a__wire_ops__L9 = a;
assign b__wire_ops__L10 = b;
assign sel__wire_ops__L11 = sel;
assign pyc_xor_3 = (a__wire_ops__L9 ^ b__wire_ops__L10);
assign COMB__y__wire_ops__L14 = pyc_xor_3;
assign pyc_and_4 = (a__wire_ops__L9 & b__wire_ops__L10);
assign COMB__y__wire_ops__L16 = pyc_and_4;
assign pyc_mux_5 = (sel__wire_ops__L11 ? COMB__y__wire_ops__L16 : COMB__y__wire_ops__L14);
assign COMB__y__wire_ops__L15 = pyc_mux_5;
assign pyc_comb_6 = pyc_constant_1;
assign pyc_comb_7 = pyc_constant_2;
assign pyc_comb_8 = COMB__y__wire_ops__L15;
pyc_reg #(.WIDTH(8)) pyc_reg_9_inst (
  .clk(sys_clk),
  .rst(sys_rst),
  .en(pyc_comb_7),
  .d(y_reg__next),
  .init(pyc_comb_6),
  .q(pyc_reg_9)
);
assign y_reg = pyc_reg_9;
assign r__wire_ops__L18 = y_reg;
assign pyc_comb_10 = r__wire_ops__L18;
assign y_reg__next = pyc_comb_8;
assign y = pyc_comb_10;

endmodule

