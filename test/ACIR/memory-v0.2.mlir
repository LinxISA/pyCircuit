...
builtin.module attributes {ac.contract_epoch = "0.3"} {
  ...
  ac.memory.instance @owned data i16 entries 16 init 0 owner "/owner" stable_id "memory/owner/owned"
  %input = ac.source depth 4 latency 1 : !ac.queue<!ac.struct<@types::@Request>>
  %response = ac.memory.request @sram, %input ordinal 0 result_field "data" depth 4 latency 1 address {
  ...
  ac.scope @owner() {
    ...
    %same_response = ac.memory.request @owned, %same_input ordinal 0 result_field "data" depth 1 latency 1 address {
    ^address(%item: !ac.var<!ac.struct<@types::@Request>>):
      %address = ac.var.get %item field "address" : !ac.var<!ac.struct<@types::@Request>> -> !ac.var<i8>
      ac.memory.yield %address : !ac.var<i8>
    } write {
    ...
    ac.scope @child() {
      ...
      %child_response = ac.memory.request @owned, %child_input ordinal 1 result_field "data" depth 1 latency 1 address {
      ^address(%item: !ac.var<!ac.struct<@types::@Request>>):
        %address = ac.var.get %item field "address" : !ac.var<!ac.struct<@types::@Request>> -> !ac.var<i8>
        ac.memory.yield %address : !ac.var<i8>
      } write {
...
// CHECK: ac.memory.instance @sram data i16 entries 16 init 0
// CHECK: ac.memory.instance @owned data i16 entries 16 init 0 owner "/owner" stable_id "memory/owner/owned"
// CHECK: ac.memory.request @sram, %{{.*}} ordinal 0 result_field "data" depth 4 latency 1
// CHECK: ac.memory.request @owned, %{{.*}} ordinal 0 result_field "data" depth 1 latency 1
// CHECK: ac.memory.request @owned, %{{.*}} ordinal 1 result_field "data" depth 1 latency 1
