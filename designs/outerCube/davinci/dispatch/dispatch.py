...
def dispatch(
    ...
    *,
    ...
    in_domain = [_in(inputs, f"domain{i}", m, domain, prefix, DOMAIN_W) for i in range(width)]
    in_op     = [_in(inputs, f"op{i}", m, domain, prefix, uop_w) for i in range(width)]
    in_is_load  = [_in(inputs, f"is_load{i}", m, domain, prefix, 1) for i in range(width)]
    in_is_store = [_in(inputs, f"is_store{i}", m, domain, prefix, 1) for i in range(width)]
    in_is_vec   = [_in(inputs, f"is_vec{i}", m, domain, prefix, 1) for i in range(width)]

    in_psrc1    = [_in(inputs, f"psrc1_{i}", m, domain, prefix, stag_w) for i in range(width)]
    in_psrc2    = [_in(inputs, f"psrc2_{i}", m, domain, prefix, stag_w) for i in range(width)]
    ...
    in_ptsrc    = [_in(inputs, f"ptsrc{i}", m, domain, prefix, ttag_w) for i in range(width)]
    in_ptdst    = [_in(inputs, f"ptdst{i}", m, domain, prefix, ttag_w) for i in range(width)]
    ...
    for i in range(width):
        ...
        is_scalar = (d == cas(domain, m.const(DOMAIN_SCALAR, width=DOMAIN_W), cycle=0)) | \
        ...
        target_full = mux(to_scalar, scalar_rs_full,
                     mux(to_lsu, lsu_rs_full,
                     mux(to_vec, vec_rs_full,
                     mux(to_cube, cube_rs_full,
                     mux(to_mte, mte_rs_full,
...
if __name__ == "__main__":
    print(compile_cycle_aware(dispatch, name="dispatch", eager=True,
                               width=2, uop_w=4, stag_w=3, ttag_w=3).emit_mlir())
