...
def davinci_top(
    ...
    *,
    ...
    dmem_rdata   = _in(inputs, "dmem_rdata",   m, domain, prefix=prefix, width=SCALAR_DATA_W)
    ...
    }, addr_w=ADDR_W, fetch_w=W, prefix=f"{prefix}_fe")
    ...
    dec_out = domain.call(decoder, inputs=dec_inputs, width=W,
                          prefix=f"{prefix}_dec")
    ...
    # ═══════════════════════════════════════════════════════════════════
    ren_inputs = {"restore": bru_redirect,
    ...
    for i in range(W):
        ...
        ren_inputs[f"sfl_tag{i}"]  = cas(domain, m.const(ARCH_GREGS + i, width=PHYS_GREG_W), cycle=0)
    ...
    ren_out = domain.call(rename, inputs=ren_inputs, width=W,
                          prefix=f"{prefix}_ren")
    ...
    ds_out = domain.call(dispatch, inputs=ds_inputs, width=W,
                         prefix=f"{prefix}_ds")
    ...
    for i in range(W):
        ...
        srs_inputs[f"disp_data1_{i}"] = cas(domain, m.const(0, width=SCALAR_DATA_W), cycle=0)
        srs_inputs[f"disp_psrc2_{i}"] = ds_psrc2[i]
        srs_inputs[f"disp_rdy2_{i}"]  = ds_rdy2[i]
        srs_inputs[f"disp_data2_{i}"] = cas(domain, m.const(0, width=SCALAR_DATA_W), cycle=0)
        srs_inputs[f"disp_pdst{i}"]   = ds_pdst[i]
        srs_inputs[f"disp_ckpt{i}"]   = cas(domain, m.const(0, width=CHECKPOINT_W), cycle=0)
    for c in range(CDB_PORTS):
        srs_inputs[f"cdb_valid{c}"] = zero1
        srs_inputs[f"cdb_tag{c}"]   = cas(domain, m.const(0, width=PHYS_GREG_W), cycle=0)
        srs_inputs[f"cdb_data{c}"]  = cas(domain, m.const(0, width=SCALAR_DATA_W), cycle=0)

    srs_out = domain.call(scalar_rs, inputs=srs_inputs,
                          n_entries=SCALAR_RS_ENTRIES, n_dispatch=W,
                          n_issue=SCALAR_ISSUE_WIDTH, n_cdb=CDB_PORTS,
    ...
    alu_out = domain.call(alu, inputs={
        ...
        "pdst":  srs_issue_pdst,
    }, data_w=SCALAR_DATA_W, prefix=f"{prefix}_alu0")

    # ═══════════════════════════════════════════════════════════════════
    #  7. MUL/DIV
    # ═══════════════════════════════════════════════════════════════════
    md_out = domain.call(muldiv, inputs={
        "valid": srs_issue_valid,
        "func":  srs_issue_op,
        "src1":  srs_issue_data1,
        "src2":  srs_issue_data2,
        "pdst":  srs_issue_pdst,
    }, data_w=SCALAR_DATA_W, prefix=f"{prefix}_md")

    # ═══════════════════════════════════════════════════════════════════
    #  8. BRU
    # ═══════════════════════════════════════════════════════════════════
    bru_out = domain.call(bru, inputs={
        "valid":     srs_issue_valid,
        "func":      srs_issue_op,
        "src1":      srs_issue_data1,
        "src2":      srs_issue_data2,
        ...
        "pdst":      srs_issue_pdst,
        "ckpt":      srs_issue_ckpt,
    }, data_w=SCALAR_DATA_W, addr_w=ADDR_W, prefix=f"{prefix}_bru")
    ...
    lrs_out = domain.call(lsu_rs, inputs=lrs_inputs,
                          n_entries=LSU_RS_ENTRIES, n_dispatch=W,
                          n_cdb=CDB_PORTS, prefix=f"{prefix}_lrs")

    lrs_ld_valid = lrs_out["ld_issue_valid"]
    lrs_ld_pdst  = lrs_out["ld_issue_pdst"]
    lrs_st_valid = lrs_out["st_issue_valid"]

    lsu_out = domain.call(lsu, inputs={
    ...
    }, data_w=SCALAR_DATA_W, addr_w=ADDR_W, prefix=f"{prefix}_lsu")
    ...
    vrs_out = domain.call(vec_rs, inputs=vrs_inputs,
                          n_entries=VEC_RS_ENTRIES, n_dispatch=W,
                          n_tcb=TCB_PORTS, n_tile_src=2,
    ...
    crs_out = domain.call(cube_rs, inputs=crs_inputs,
                          n_entries=CUBE_RS_ENTRIES, n_dispatch=W,
                          n_tcb=TCB_PORTS, n_tile_src=2,
    ...
    mrs_out = domain.call(mte_rs, inputs=mrs_inputs,
                          n_entries=MTE_RS_ENTRIES, n_dispatch=W,
                          n_cdb=CDB_PORTS, n_tcb=TCB_PORTS,
