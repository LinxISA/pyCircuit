...
def ibuf(
    ...
    *,
    ...
    # ── Cycle 0: Inputs ──────────────────────────────────────────────
    enq_valid = [_in(inputs, f"enq_valid{i}", m, domain, prefix, 1)
                 for i in range(width)]
    enq_instr = [_in(inputs, f"enq_instr{i}", m, domain, prefix, instr_w)
                 for i in range(width)]
    enq_pc    = [_in(inputs, f"enq_pc{i}", m, domain, prefix, addr_w)
                 for i in range(width)]
    ...
    # ── State ────────────────────────────────────────────────────────
    buf_valid = [domain.signal(width=1, reset_value=0, name=f"{prefix}_v_{i}")
                 for i in range(depth)]
    buf_instr = [domain.signal(width=instr_w, reset_value=0, name=f"{prefix}_ins_{i}")
                 for i in range(depth)]
    buf_pc    = [domain.signal(width=addr_w, reset_value=0, name=f"{prefix}_pc_{i}")
                 for i in range(depth)]
    ...
    n_deq_max = mux(count < cas(domain, m.const(width, width=cnt_w), cycle=0), count, cas(domain, m.const(width, width=cnt_w), cycle=0))
    ...
    for i in range(width):
        ...
        for s in range(depth):
            hit = enq_valid[i] & (wr_idx == cas(domain, m.const(s, width=ptr_w), cycle=0)) & (~flush)
    ...
    for i in range(width):
        ...
        do_deq = deq_ready & (count > cas(domain, m.const(i, width=cnt_w), cycle=0)) & (~flush)
        for s in range(depth):
    ...
    next_tail = mux(flush, cas(domain, m.const(0, width=ptr_w), cycle=0),
                    (tail + n_enq).trunc(ptr_w))
    next_head = mux(flush, cas(domain, m.const(0, width=ptr_w), cycle=0),
                    (head + n_deq).trunc(ptr_w))
    next_count = mux(flush, cas(domain, m.const(0, width=cnt_w), cycle=0),
...
if __name__ == "__main__":
    print(compile_cycle_aware(ibuf, name="ibuf", eager=True,
                               depth=4, width=2, addr_w=32).emit_mlir())
