...
def checkpoint(
    ...
    *,
    ...
    save_smap = [_in(inputs, f"save_smap{i}", m, domain, prefix, sphys_w) for i in range(n_sarch)]
    save_srdy = [_in(inputs, f"save_srdy{i}", m, domain, prefix, 1) for i in range(n_sarch)]
    # Tile RAT snapshot to save
    save_tmap = [_in(inputs, f"save_tmap{i}", m, domain, prefix, tphys_w) for i in range(n_tarch)]
    save_trdy = [_in(inputs, f"save_trdy{i}", m, domain, prefix, 1) for i in range(n_tarch)]
    # Pointers
    ...
    occupied  = [domain.signal(width=1, reset_value=0, name=f"{prefix}_occ_{s}") for s in range(n_slots)]

    # Checkpoint storage (scalar RAT)
    ckpt_smap = [[domain.signal(width=sphys_w, reset_value=a, name=f"{prefix}_smap_{s}_{a}")
                   for a in range(n_sarch)] for s in range(n_slots)]
    ckpt_srdy = [[domain.signal(width=1, reset_value=1, name=f"{prefix}_srdy_{s}_{a}")
                   for a in range(n_sarch)] for s in range(n_slots)]

    # Checkpoint storage (tile RAT)
    ckpt_tmap = [[domain.signal(width=tphys_w, reset_value=a, name=f"{prefix}_tmap_{s}_{a}")
                   for a in range(n_tarch)] for s in range(n_slots)]
    ckpt_trdy = [[domain.signal(width=1, reset_value=1, name=f"{prefix}_trdy_{s}_{a}")
                   for a in range(n_tarch)] for s in range(n_slots)]

    # Pointer storage
    ckpt_sfl = [domain.signal(width=sfl_ptr_w, reset_value=0, name=f"{prefix}_sfl_{s}") for s in range(n_slots)]
    ckpt_tfl = [domain.signal(width=tfl_ptr_w, reset_value=0, name=f"{prefix}_tfl_{s}") for s in range(n_slots)]
    ckpt_ras = [domain.signal(width=ras_ptr_w, reset_value=0, name=f"{prefix}_ras_{s}") for s in range(n_slots)]
    ...
    for s in range(n_slots):
        ...
        occupied[s] <<= mux(is_alloc, cas(domain, m.const(1, width=1), cycle=0),
                             mux(is_dealloc, cas(domain, m.const(0, width=1), cycle=0), occupied[s]))

    # Advance alloc pointer
    next_ptr = (alloc_ptr + cas(domain, m.const(1, width=slot_w), cycle=0)).trunc(slot_w)
...
if __name__ == "__main__":
    print(compile_cycle_aware(checkpoint, name="checkpoint", eager=True,
                               n_slots=4, slot_w=2, n_sarch=4, n_tarch=4,
                               sphys_w=3, tphys_w=3, prefix="ckpt").emit_mlir())
