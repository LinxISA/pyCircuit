...
def scalar_rat(
    ...
    *,
    ...
    src1_arch = [_in(inputs, f"src1_arch{i}", m, domain, prefix, arch_w) for i in range(width)]
    src2_arch = [_in(inputs, f"src2_arch{i}", m, domain, prefix, arch_w) for i in range(width)]
    ...
    # Writeback (set ready)
    wb_valid = [_in(inputs, f"wb_valid{i}", m, domain, prefix, 1) for i in range(6)]  # CDB ports
...
if __name__ == "__main__":
    print(compile_cycle_aware(scalar_rat, name="scalar_rat", eager=True,
                               n_arch=8, arch_w=3, phys_w=4, width=2, prefix="srat").emit_mlir())
