...
def alu(
    ...
    *,
    ...
    result = zero
    result = mux(func == cas(domain, m.const(ALU_ADD, width=func_w), cycle=0), r_add, result)
    result = mux(func == cas(domain, m.const(ALU_SUB, width=func_w), cycle=0), r_sub, result)
    result = mux(func == cas(domain, m.const(ALU_AND, width=func_w), cycle=0), r_and, result)
    result = mux(func == cas(domain, m.const(ALU_OR,  width=func_w), cycle=0), r_or,  result)
    result = mux(func == cas(domain, m.const(ALU_XOR, width=func_w), cycle=0), r_xor, result)
    result = mux(func == cas(domain, m.const(ALU_SLL, width=func_w), cycle=0), r_sll, result)
    result = mux(func == cas(domain, m.const(ALU_SRL, width=func_w), cycle=0), r_srl, result)
    result = mux(func == cas(domain, m.const(ALU_SRA, width=func_w), cycle=0), r_sra, result)
    result = mux(func == cas(domain, m.const(ALU_SLT, width=func_w), cycle=0), r_slt, result)
    result = mux(func == cas(domain, m.const(ALU_SLTU,width=func_w), cycle=0), r_sltu,result)
    result = mux(func == cas(domain, m.const(ALU_LUI, width=func_w), cycle=0), src2,  result)
    result = mux(func == cas(domain, m.const(ALU_MOV, width=func_w), cycle=0), src1,  result)
