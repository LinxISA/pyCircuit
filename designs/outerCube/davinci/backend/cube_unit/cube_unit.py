...
def cube_unit(
    ...
    *,
    ...
    drain_ctr = domain.signal(width=4, reset_value=0, name=f"{prefix}_dctr")  # drain epoch counter
    ...
    opa_active = busy & is_opa & (remain > cas(domain, m.const(0, width=cnt_w), cycle=0))

    # Drain in progress: writing to TRegFile
    drain_active = busy & is_drain & (drain_ctr < cas(domain, m.const(epoch_cy, width=4), cycle=0))

    # Completion
    opa_done = busy & is_opa & (remain == cas(domain, m.const(0, width=cnt_w), cycle=0))
    drain_done = busy & is_drain & (drain_ctr >= cas(domain, m.const(epoch_cy, width=4), cycle=0))
    ...
    start_opa   = issue_valid & (~busy) & (issue_op == cas(domain, m.const(CUBE_OPA, width=op_w), cycle=0))
    start_drain = issue_valid & (~busy) & (issue_op == cas(domain, m.const(CUBE_DRAIN, width=op_w), cycle=0))
    start_zero  = issue_valid & (~busy) & (issue_op == cas(domain, m.const(CUBE_ZERO, width=op_w), cycle=0))

    # OPA: decrement remaining B-tile count each epoch
    new_remain = mux(opa_active, (remain - cas(domain, m.const(1, width=cnt_w), cycle=0)).trunc(cnt_w), remain)

    # Drain: count write epoch cycles
    new_drain_ctr = mux(drain_active,
                         (drain_ctr + cas(domain, m.const(1, width=4), cycle=0)).trunc(4),
                         drain_ctr)

    # Transitions
    new_busy = busy
    new_busy = mux(opa_done | drain_done | start_zero, cas(domain, m.const(0, width=1), cycle=0), new_busy)
    new_busy = mux(start_opa | start_drain, cas(domain, m.const(1, width=1), cycle=0), new_busy)

    busy <<= new_busy
    cur_op <<= mux(start_opa, cas(domain, m.const(CUBE_OPA, width=op_w), cycle=0),
    ...
    drain_ctr <<= mux(start_drain, cas(domain, m.const(0, width=4), cycle=0), new_drain_ctr)
