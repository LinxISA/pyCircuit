...
// I × A = A (where I is identity matrix)
static bool testIdentity() {
  ...
  if (trace_log || trace_vcd) {
    ...
    std::filesystem::path out_dir = trace_dir_env ? std::filesystem::path(trace_dir_env) : std::filesystem::path(".pycircuit_out/janus/janus_cube_pyc");
