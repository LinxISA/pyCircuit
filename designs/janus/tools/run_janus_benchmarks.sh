...
BENCH_DIR="${ROOT_DIR}/.pycircuit_out/janus/benchmarks"
mkdir -p "${BENCH_DIR}"

CORE_SRC="${ROOT_DIR}/designs/janus/programs_src/coremark_lite.S"
DHRY_SRC="${ROOT_DIR}/designs/janus/programs_src/dhrystone_lite.S"
...
bash "${ROOT_DIR}/designs/janus/tools/build_linx_memh.sh" "${CORE_SRC}" "${CORE_MEMH}" >/dev/null
bash "${ROOT_DIR}/designs/janus/tools/build_linx_memh.sh" "${DHRY_SRC}" "${DHRY_MEMH}" >/dev/null
...
run_cpp() {
  ...
  PYC_TRACE_DIR="${trace_dir}" PYC_MAX_CYCLES="${MAX_CYCLES}" PYC_KONATA=1 \
    bash "${ROOT_DIR}/designs/janus/tools/run_janus_bcc_ooo_pyc_cpp.sh" "${memh}" 2>&1
}
...
run_vlt() {
  ...
  bash "${ROOT_DIR}/designs/janus/tools/run_janus_bcc_ooo_pyc_verilator.sh" \
    +notrace +nolog \
    +konata="${konata_path}" \
