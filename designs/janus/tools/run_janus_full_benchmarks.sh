...
BENCH_DIR="${ROOT_DIR}/.pycircuit_out/janus/benchmarks_full"
...
build_out="$(
  ...
  DHRY_RUNS="${DHRY_RUNS}" \
  bash "${ROOT_DIR}/designs/janus/tools/build_linxisa_benchmarks_memh.sh"
)"
CORE_MEMH="$(printf "%s\n" "${build_out}" | sed -n '1p')"
DHRY_MEMH="$(printf "%s\n" "${build_out}" | sed -n '2p')"
...
run_cpp() {
  ...
  PYC_BOOT_SP="${BOOT_SP}" \
  bash "${ROOT_DIR}/designs/janus/tools/run_janus_bcc_ooo_pyc_cpp.sh" "${memh}" "$@"
}
