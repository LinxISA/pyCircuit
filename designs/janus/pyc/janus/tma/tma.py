...
def build_tma(m: Circuit, *, clk: Signal, rst: Signal, launch_valid: Wire, launch_brob: Wire, launch_payload: Wire, latency: int=3, name: str='tma') -> TmaOut:
    if latency <= 0:
    ...
    tile_data = payload.out() + c(64, width=64)
    return TmaOut(ready=ready, done_valid=finishing, done_brob=brob.out(), tile_write_valid=finishing, tile_write_idx=tile_idx, tile_write_data=tile_data)
