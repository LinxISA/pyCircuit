...
def build_tau(m: Circuit, *, clk: Signal, rst: Signal, launch_valid: Wire, launch_brob: Wire, launch_payload: Wire, latency: int=4, name: str='tau') -> TauOut:
    if latency <= 0:
    ...
    src_a = unsigned(payload.out()[16:32])
    src_b = unsigned(payload.out()[32:48])
    sub_res = src_a + (~src_b + c(1, width=64))
    tau_res = sub_res if op_sub else src_a + src_b
    tile_idx = payload.out()[0:6]
    return TauOut(ready=ready, done_valid=finishing, done_brob=brob.out(), tile_write_valid=finishing, tile_write_idx=tile_idx, tile_write_data=tau_res)
