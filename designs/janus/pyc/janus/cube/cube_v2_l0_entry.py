...
from janus.cube.cube_v2_consts import ARRAY_SIZE, INPUT_WIDTH
...
def build_l0_entry(m: Circuit) -> None:
    ...
    for row in range(ARRAY_SIZE):
        ...
        for col in range(ARRAY_SIZE):
            reg = m.out(f'd_r{row}_c{col}', clk=clk, rst=rst, width=INPUT_WIDTH, init=0, en=c(1, width=1))
            row_regs.append(reg)
    ...
    last_elem = load_valid & (load_row == c(ARRAY_SIZE - 1, width=4)) & (load_col == c(ARRAY_SIZE - 1, width=4))
