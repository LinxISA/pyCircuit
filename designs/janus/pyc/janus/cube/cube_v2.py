...
from janus.cube.cube_v2_l0 import build_l0a, build_l0b
from janus.cube.cube_v2_mmio import build_mmio_inst_write, build_mmio_read, build_mmio_write
from janus.cube.cube_v2_systolic import build_pipelined_systolic_array
...
def build(m: Circuit, *, base_addr: int=2147483648) -> None:
    ...
    l0a_status, l0a_matrix, l0a_load_done = build_l0a(m, clk=clk, rst=rst, consts=consts, load_entry_idx=l0a_entry_idx, load_row=l0a_row, load_col=l0a_col, load_data=mem_wdata, load_valid=l0a_load_valid, read_entry_idx=c(0, width=L0_IDX_WIDTH))
    l0b_status, l0b_matrix, l0b_load_done = build_l0b(m, clk=clk, rst=rst, consts=consts, load_entry_idx=l0b_entry_idx, load_row=l0b_row, load_col=l0b_col, load_data=mem_wdata, load_valid=l0b_load_valid, read_entry_idx=c(0, width=L0_IDX_WIDTH))
    ...
    pipe_regs, sa_write_valid, sa_results, sa_write_acc_idx, sa_write_is_first, sa_write_is_last, sa_busy = build_pipelined_systolic_array(m, clk=clk, rst=rst, consts=consts, issue_valid=issue_result.issue_valid, issue_uop=issue_result.uop, l0a_data=l0a_matrix, l0b_data=l0b_matrix, stall=consts.zero1)
