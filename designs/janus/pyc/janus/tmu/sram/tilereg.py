...
from pycircuit import Circuit, Reg, Wire, unsigned
...
def build_tilereg(m: Circuit, *, clk: Signal, rst: Signal, read_idx: Wire, write_valid: Wire, write_idx: Wire, write_data: Wire, num_tiles: int=64, name: str='tilereg', use_byte_mem: bool=False) -> TileRegFile:
    if num_tiles <= 0:
    ...
    if use_byte_mem:
        ...
        read_data = m.byte_mem(clk=clk, rst=rst, raddr=byte_addr, wvalid=write_valid, waddr=byte_waddr, wdata=write_data, wstrb=c(255, width=8), depth=depth_bytes, name=name)
        cells: list[Reg] = []
    else:
        with m.scope(name):
            cells = []
        ...
        for i in range(num_tiles):
            hit = write_valid & (widx == c(i, width=widx.width))
