...
def build(m: Circuit, *, mem_bytes: int=1 << 20) -> None:
    ...
    bctrl = build_bctrl(m, clk=clk, rst=rst, cmd_valid=m.wire(bcc.block_cmd_valid), cmd_kind=m.wire(bcc.block_cmd_kind), cmd_payload=m.wire(bcc.block_cmd_payload), cmd_tile=m.wire(bcc.block_cmd_tile), cmd_tag=m.wire(bcc.block_cmd_tag), tma_ready=tma_ready.out(), cube_ready=cube_ready.out(), tau_ready=tau_ready.out(), tma_done_valid=tma_done_valid.out(), tma_done_brob=tma_done_brob.out(), cube_done_valid=cube_done_valid.out(), cube_done_brob=cube_done_brob.out(), tau_done_valid=tau_done_valid.out(), tau_done_brob=tau_done_brob.out())
    mask_target = c(18446744073709551423, width=64)
    ...
    node0 = build_ring_node(m, in_valid=c(0, width=1), in_brob=c(0, width=8), in_payload=c(0, width=64), inject_valid=bctrl.dispatch_tma_valid, inject_brob=bctrl.dispatch_tma_brob, inject_payload=tma_payload, node_id=0, local_ready=tma_ready.out())
    ...
    node1 = build_ring_node(m, in_valid=pipe0.valid, in_brob=pipe0.brob, in_payload=pipe0.payload, inject_valid=bctrl.dispatch_cube_valid, inject_brob=bctrl.dispatch_cube_brob, inject_payload=cube_payload, node_id=1, local_ready=cube_ready.out())
    ...
    node2 = build_ring_node(m, in_valid=pipe1.valid, in_brob=pipe1.brob, in_payload=pipe1.payload, inject_valid=bctrl.dispatch_tau_valid, inject_brob=bctrl.dispatch_tau_brob, inject_payload=tau_payload, node_id=2, local_ready=tau_ready.out())
    ...
    tma = build_tma(m, clk=clk, rst=rst, launch_valid=node0.local_valid, launch_brob=node0.local_brob, launch_payload=node0.local_payload)
    cube = build_cube(m, clk=clk, rst=rst, launch_valid=node1.local_valid, launch_brob=node1.local_brob, launch_payload=node1.local_payload)
    tau = build_tau(m, clk=clk, rst=rst, launch_valid=node2.local_valid, launch_brob=node2.local_brob, launch_payload=node2.local_payload)
    ...
    tile_write_idx = tma.tile_write_idx if tma.tile_write_valid else cube.tile_write_idx if cube.tile_write_valid else tau.tile_write_idx
    tile_write_data = tma.tile_write_data if tma.tile_write_valid else cube.tile_write_data if cube.tile_write_valid else tau.tile_write_data
    tile_regs = build_tilereg(m, clk=clk, rst=rst, read_idx=m.wire(bcc.block_cmd_tile), write_valid=tile_write_valid, write_idx=tile_write_idx, write_data=tile_write_data)
