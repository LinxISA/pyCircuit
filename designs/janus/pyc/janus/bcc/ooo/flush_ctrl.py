...
def arbitrate_flush(m: Circuit, *, bru_redirect_valid: Wire, bru_redirect_pc: Wire, lsu_nuke_valid: Wire, lsu_nuke_pc: Wire, exc_valid: Wire, exc_pc: Wire) -> FlushOut:
    c = m.const
    ...
    flush_pc = bru_redirect_pc if bru_redirect_valid else flush_pc
    flush_pc = lsu_nuke_pc if lsu_nuke_valid else flush_pc
    flush_pc = exc_pc if exc_valid else flush_pc
