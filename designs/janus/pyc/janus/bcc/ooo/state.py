...
def make_core_ctrl_regs(m: Circuit, clk: Signal, rst: Signal, *, boot_pc: Wire, consts: Consts) -> CoreCtrlRegs:
    ...
    with m.scope('state'):
        ...
        macro_phase = m.out('macro_phase', clk=clk, rst=rst, width=2, init=c(0, width=2), en=consts.one1)
    ...
    return CoreCtrlRegs(halted=halted, cycles=cycles, pc=pc, fpc=fpc, br_kind=br_kind, br_base_pc=br_base_pc, br_off=br_off, commit_cond=commit_cond, commit_tgt=commit_tgt, flush_pending=flush_pending, flush_pc=flush_pc, macro_active=macro_active, macro_wait_commit=macro_wait_commit, macro_phase=macro_phase, macro_op=macro_op, macro_begin=macro_begin, macro_end=macro_end, macro_stacksize=macro_stacksize, macro_reg=macro_reg, macro_i=macro_i, macro_sp_base=macro_sp_base)
...
def make_rob_regs(m: Circuit, clk: Signal, rst: Signal, *, consts: Consts, p: OooParams) -> RobRegs:
    ...
    return RobRegs(head=head, tail=tail, count=count, valid=valid, done=done, op=op, len_bytes=len_bytes, dst_kind=dst_kind, dst_areg=dst_areg, pdst=pdst, value=value, store_addr=store_addr, store_data=store_data, store_size=store_size, is_store=is_store, load_addr=load_addr, load_data=load_data, load_size=load_size, is_load=is_load, insn_raw=insn_raw, macro_begin=macro_begin, macro_end=macro_end)
...
def make_iq_regs(m: Circuit, clk: Signal, rst: Signal, *, consts: Consts, p: OooParams, name: str='iq') -> IqRegs:
    ...
    with m.scope(name):
        ...
        for i in range(p.iq_depth):
            ...
            srcr_type.append(m.out(f'st{i}', clk=clk, rst=rst, width=2, init=c(0, width=2), en=consts.one1))
    ...
    return IqRegs(valid=valid, rob=rob, op=op, pc=pc, imm=imm, srcl=srcl, srcr=srcr, srcr_type=srcr_type, shamt=shamt, srcp=srcp, pdst=pdst, has_dst=has_dst)
