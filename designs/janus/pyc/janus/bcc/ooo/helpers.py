...
def mux_by_uindex(m: Circuit, *, idx: Wire, items: list[Wire | Reg], default: Wire) -> Wire:
    ...
    for i, it in enumerate(items):
        ...
        sel = idx == c(i, width=idx.width)
        out = sel._select_internal(w, out)
    return out
...
def mask_bit(m: Circuit, *, mask: Wire, idx: Wire, width: int) -> Wire:
    ...
    for i in range(int(width)):
        sel = idx == c(i, width=idx.width)
        out = sel._select_internal(mask[i], out)
    return out
...
def onehot_from_tag(m: Circuit, *, tag: Wire, width: int, tag_width: int) -> Wire:
    ...
    out = c(0, width=width)
    for i in range(int(width)):
        sel = tag == c(i, width=tag_width)
        out = sel._select_internal(c(1 << i, width=width), out)
    return out
