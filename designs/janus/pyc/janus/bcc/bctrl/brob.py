...
def brob_head(m: Circuit, b: BrobRegs) -> BrobHead:
    idx = b.head.out()
    return BrobHead(valid=mux_by_uindex(m, idx=idx, items=b.valid, default=u(1, 0)), done=mux_by_uindex(m, idx=idx, items=b.done, default=u(1, 0)), pe_sel=mux_by_uindex(m, idx=idx, items=b.pe_sel, default=u(2, 0)), tag=mux_by_uindex(m, idx=idx, items=b.tag, default=u(8, 0)), tile=mux_by_uindex(m, idx=idx, items=b.tile, default=u(6, 0)), payload=mux_by_uindex(m, idx=idx, items=b.payload, default=u(64, 0)), idx=idx)

def brob_has_space(m: Circuit, b: BrobRegs, *, depth: int) -> Wire:
    return b.count.out().ult(u(b.count.width, depth))
