...
def build_bctrl(m: Circuit, *, clk: Signal, rst: Signal, cmd_valid: Wire, cmd_kind: Wire, cmd_payload: Wire, cmd_tile: Wire, cmd_tag: Wire, tma_ready: Wire, cube_ready: Wire, tau_ready: Wire, tma_done_valid: Wire, tma_done_brob: Wire, cube_done_valid: Wire, cube_done_brob: Wire, tau_done_valid: Wire, tau_done_brob: Wire, bisq_depth: int=16, brob_depth: int=32) -> BCtrlOutputs:
    c = m.const
    ...
    pe_sel = c(0, width=2) if cmd_kind == c(0, width=2) else c(2, width=2)
    pe_sel = c(1, width=2) if cmd_kind == c(1, width=2) else pe_sel
    ...
    qh = bisq_head(m, bisq)
    head_ready = tma_ready if qh.pe_sel == c(0, width=2) else cube_ready if qh.pe_sel == c(1, width=2) else tau_ready
    dispatch_fire = qh.valid & head_ready
    dispatch_tma_valid = dispatch_fire & (qh.pe_sel == c(0, width=2))
    dispatch_cube_valid = dispatch_fire & (qh.pe_sel == c(1, width=2))
    dispatch_tau_valid = dispatch_fire & (qh.pe_sel == c(2, width=2))
    done_any = tma_done_valid | cube_done_valid | tau_done_valid
    done_brob_u8 = tma_done_brob if tma_done_valid else cube_done_brob if cube_done_valid else tau_done_brob
    ...
    for i in range(bisq_depth):
        ...
        push_hit = cmd_accept & (bisq.tail.out() == idx)
        pop_hit = dispatch_fire & (bisq.head.out() == idx)
        valid_next = bisq.valid[i].out()
        valid_next = c(0, width=1) if pop_hit else valid_next
        valid_next = c(1, width=1) if push_hit else valid_next
        bisq.valid[i].set(valid_next)
        bid_next = bisq.brob_idx[i].out()
        bid_next = _fit_u8(m, brob.tail.out()) if push_hit else bid_next
        bisq.brob_idx[i].set(bid_next)
        pe_next = bisq.pe_sel[i].out()
        pe_next = pe_sel if push_hit else pe_next
        bisq.pe_sel[i].set(pe_next)
        payload_next = bisq.payload[i].out()
        payload_next = renamed_payload if push_hit else payload_next
        bisq.payload[i].set(payload_next)
    bisq_head_next = bisq.head.out() + c(1, width=bisq.head.width) if dispatch_fire else bisq.head.out()
    bisq_tail_next = bisq.tail.out() + c(1, width=bisq.tail.width) if cmd_accept else bisq.tail.out()
    ...
    bisq_cnt_next = bisq_cnt_next + plus1_bisq if only_push else bisq_cnt_next
    bisq_cnt_next = bisq_cnt_next + minus1_bisq if only_pop else bisq_cnt_next
    ...
    for i in range(brob_depth):
        ...
        alloc_hit = cmd_accept & (brob.tail.out() == idx)
        done_hit = done_any & (done_brob_idx == idx)
        retire_hit = retire_valid & (brob.head.out() == idx)
        valid_next = brob.valid[i].out()
        valid_next = c(0, width=1) if retire_hit else valid_next
        valid_next = c(1, width=1) if alloc_hit else valid_next
        brob.valid[i].set(valid_next)
        done_next = brob.done[i].out()
        done_next = c(0, width=1) if retire_hit else done_next
        done_next = c(0, width=1) if alloc_hit else done_next
        done_next = c(1, width=1) if done_hit else done_next
        brob.done[i].set(done_next)
        pe_next = brob.pe_sel[i].out()
        pe_next = pe_sel if alloc_hit else pe_next
        brob.pe_sel[i].set(pe_next)
        tag_next = brob.tag[i].out()
        tag_next = _fit_u8(m, cmd_tag) if alloc_hit else tag_next
        brob.tag[i].set(tag_next)
        tile_next = brob.tile[i].out()
        tile_next = _fit_idx(physical_tile, width=6) if alloc_hit else tile_next
        brob.tile[i].set(tile_next)
        payload_next = brob.payload[i].out()
        payload_next = renamed_payload if alloc_hit else payload_next
        brob.payload[i].set(payload_next)
    brob_head_next = brob.head.out() + c(1, width=brob.head.width) if retire_valid else brob.head.out()
    brob_tail_next = brob.tail.out() + c(1, width=brob.tail.width) if cmd_accept else brob.tail.out()
    ...
    brob_cnt_next = brob_cnt_next + plus1_brob if only_alloc else brob_cnt_next
    brob_cnt_next = brob_cnt_next + minus1_brob if only_retire else brob_cnt_next
    brob.head.set(brob_head_next)
    brob.tail.set(brob_tail_next)
    brob.count.set(brob_cnt_next)
    next_phys = brenu.next_phys.out()
    next_phys = next_phys + c(1, width=next_phys.width) if cmd_accept else next_phys
    brenu.next_phys.set(next_phys)
    for i in range(len(brenu.tile_map)):
        idx = c(i, width=cmd_tile.width)
        hit = cmd_accept & (cmd_tile == idx)
        map_next = brenu.tile_map[i].out()
        map_next = physical_tile if hit else map_next
    ...
    return BCtrlOutputs(cmd_accept=cmd_accept, dispatch_tma_valid=dispatch_tma_valid, dispatch_tma_brob=qh.brob_idx, dispatch_tma_payload=qh.payload, dispatch_cube_valid=dispatch_cube_valid, dispatch_cube_brob=qh.brob_idx, dispatch_cube_payload=qh.payload, dispatch_tau_valid=dispatch_tau_valid, dispatch_tau_brob=qh.brob_idx, dispatch_tau_payload=qh.payload, retire_valid=retire_valid, retire_tag=bh.tag, retire_tile=bh.tile, retire_pe=bh.pe_sel, bisq_count=bisq.count.out(), brob_count=brob.count.out())
