...
def bisq_head(m: Circuit, q: BisqRegs) -> BisqHead:
    return BisqHead(valid=mux_by_uindex(m, idx=q.head.out(), items=q.valid, default=u(1, 0)), brob_idx=mux_by_uindex(m, idx=q.head.out(), items=q.brob_idx, default=u(8, 0)), pe_sel=mux_by_uindex(m, idx=q.head.out(), items=q.pe_sel, default=u(2, 0)), payload=mux_by_uindex(m, idx=q.head.out(), items=q.payload, default=u(64, 0)))

def bisq_has_space(m: Circuit, q: BisqRegs, *, depth: int) -> Wire:
    return q.count.out().ult(u(q.count.width, depth))
