...
from pycircuit import Circuit, Reg, Wire, u, unsigned
...
def lookup_tile(m: Circuit, r: BrenuRegs, logical_tile: Wire) -> Wire:
    return mux_by_uindex(m, idx=logical_tile, items=r.tile_map, default=u(r.next_phys.width, 0))
...
def rewrite_payload_tile(m: Circuit, *, payload: Wire, physical_tile: Wire, tile_bits: int=6) -> Wire:
    ...
    clear = payload & u(payload.width, ~mask)
    return clear | unsigned(physical_tile)
