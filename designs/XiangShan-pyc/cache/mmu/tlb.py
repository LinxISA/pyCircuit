...
def build_tlb(
    ...
    *,
    ...
    for i in range(n_ways):
        ...
        entry_vpn[i] <<= do_write.select(refill_vpn.wire, entry_vpn[i].wire)
        entry_ppn[i] <<= do_write.select(refill_ppn.wire, entry_ppn[i].wire)
        entry_asid[i] <<= do_write.select(refill_asid.wire, entry_asid[i].wire)
    ...
    replace_ptr <<= refill_valid.wire.select(next_ptr, rptr)
