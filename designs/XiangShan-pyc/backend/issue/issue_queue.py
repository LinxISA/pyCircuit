...
def issue_queue(
    ...
    *,
    ...
    enq_valid = _vin(m, domain, _in, "enq_valid", prefix=prefix, width=1, shape=enq_shape)
    enq_pdest = _vin(m, domain, _in, "enq_pdest", prefix=prefix, width=ptag_w, shape=enq_shape)
    enq_psrc1 = _vin(m, domain, _in, "enq_psrc1", prefix=prefix, width=ptag_w, shape=enq_shape)
    enq_psrc2 = _vin(m, domain, _in, "enq_psrc2", prefix=prefix, width=ptag_w, shape=enq_shape)
    enq_src1_ready = _vin(
    ...
    enq_fu_type = _vin(
        m, domain, _in, "enq_fu_type", prefix=prefix, width=fu_type_width, shape=enq_shape
    )

    wb_valid = _vin(m, domain, _in, "wb_valid", prefix=prefix, width=1, shape=wb_shape)
    wb_pdest = _vin(m, domain, _in, "wb_pdest", prefix=prefix, width=ptag_w, shape=wb_shape)
    ...
    ent_valid = domain.signal(width=1, shape=[entries], reset_value=0, name=f"{prefix}_ev")
    ent_pdest = domain.signal(width=ptag_w, shape=[entries], reset_value=0, name=f"{prefix}_epd")
    ent_psrc1 = domain.signal(width=ptag_w, shape=[entries], reset_value=0, name=f"{prefix}_eps1")
    ent_psrc2 = domain.signal(width=ptag_w, shape=[entries], reset_value=0, name=f"{prefix}_eps2")
    ent_s1rdy = domain.signal(width=1, shape=[entries], reset_value=0, name=f"{prefix}_er1")
    ent_s2rdy = domain.signal(width=1, shape=[entries], reset_value=0, name=f"{prefix}_er2")
    ent_rob_idx = domain.signal(width=rob_idx_w, shape=[entries], reset_value=0, name=f"{prefix}_erob")
    ent_fu_type = domain.signal(width=fu_type_width, shape=[entries], reset_value=0, name=f"{prefix}_efu")
    ...
    for p in range(enq_ports):
        ...
        for i in range(entries):
            we_i = do_enq & (enq_slot_idx[p] == cas(domain, m.const(i, width=idx_w), cycle=0))
            for j in range(entries):
                if j == i:
                    continue
