...
OUT_ROOT="$(pyc_out_root)/examples"
mkdir -p "${OUT_ROOT}"
...
emit_one() {
  ...
  # Emit MLIR to a temp file (repo-clean: keep generated files out-of-tree).
  local tmp_pyc
...
# Python examples.
emit_one counter "${ROOT_DIR}/designs/examples/counter.py"
emit_one counter_testgen "${ROOT_DIR}/designs/examples/counter_testgen.py"
emit_one fifo_loopback "${ROOT_DIR}/designs/examples/fifo_loopback.py"
emit_one issue_queue_2picker "${ROOT_DIR}/designs/examples/issue_queue_2picker.py"
emit_one multiclock_regs "${ROOT_DIR}/designs/examples/multiclock_regs.py"
emit_one wire_ops "${ROOT_DIR}/designs/examples/wire_ops.py"
emit_one hier_modules "${ROOT_DIR}/designs/examples/hier_modules.py"
emit_one jit_control_flow "${ROOT_DIR}/designs/examples/jit_control_flow.py"
emit_one jit_pipeline_vec "${ROOT_DIR}/designs/examples/jit_pipeline_vec.py"
emit_one calculator "${ROOT_DIR}/designs/examples/calculator/calculator.py"
emit_one digital_clock "${ROOT_DIR}/designs/examples/digital_clock/digital_clock.py"

# FastFwd: allow overriding FE count at JIT time.
# Example:
#   FASTFWD_N_FE=8 bash designs/examples/update_generated.sh
FASTFWD_EMIT_ARGS=()
if [[ -n "${FASTFWD_N_FE:-}" ]]; then
  FASTFWD_EMIT_ARGS+=(--param "N_FE=${FASTFWD_N_FE}")
fi
if (( ${#FASTFWD_EMIT_ARGS[@]} )); then
  emit_one fastfwd_pyc "${ROOT_DIR}/designs/examples/fastfwd_pyc/fastfwd_pyc.py" "${FASTFWD_EMIT_ARGS[@]}"
else
  emit_one fastfwd_pyc "${ROOT_DIR}/designs/examples/fastfwd_pyc/fastfwd_pyc.py"
fi

# LinxISA CPU (pyCircuit).
emit_one linx_cpu_pyc "${ROOT_DIR}/designs/examples/linx_cpu_pyc/linx_cpu_pyc.py"
...
mv -f "${OUT_ROOT}/calculator/calculator.hpp" "${OUT_ROOT}/calculator/calculator_gen.hpp"
mv -f "${OUT_ROOT}/digital_clock/digital_clock.hpp" "${OUT_ROOT}/digital_clock/digital_clock_gen.hpp"
...
emit_fastfwd_exam_top() {
  ...
  cat >"${outfile}" <<EOF
// Generated by pyCircuit designs/examples/update_generated.sh
...
// - runtime/verilog/*.v             (pyCircuit Verilog primitives, used via Verilog include directives in fastfwd_pyc.v)
//
module EXAM2021_TOP(
  input clk,
  input rst_n,
...
if [[ "${FASTFWD_N_FE}" == "0" ]]; then
  ...
  cp -f "${ROOT_DIR}/designs/examples/fastfwd_pyc/fe.v" "${FASTFWD_OUTDIR}/fe.v"
