...
module tc_mode2d_sanity_rand_timing;
    reg         clk;
    reg         rst_n;
    reg         vld;
    reg  [1:0]  mode;
    ...
    initial begin
        ...
        $readmemh({gen_dir, "/tc_mode2d_sanity_rand_timing_tx_vld.mem"}, tx_vld);
        $readmemh({gen_dir, "/tc_mode2d_sanity_rand_timing_tx_a.mem"}, tx_a);
        $readmemh({gen_dir, "/tc_mode2d_sanity_rand_timing_tx_b.mem"}, tx_b);
        $readmemh({gen_dir, "/tc_mode2d_sanity_rand_timing_tx_b1.mem"}, tx_b1);
        $readmemh({gen_dir, "/tc_mode2d_sanity_rand_timing_tx_e1a.mem"}, tx_e1a);
        $readmemh({gen_dir, "/tc_mode2d_sanity_rand_timing_tx_e1b0.mem"}, tx_e1b0);
        $readmemh({gen_dir, "/tc_mode2d_sanity_rand_timing_tx_e1b1.mem"}, tx_e1b1);
        $readmemh({gen_dir, "/tc_mode2d_sanity_rand_timing_exp_o0.mem"}, exp_o0);
        $readmemh({gen_dir, "/tc_mode2d_sanity_rand_timing_exp_o1.mem"}, exp_o1);
        $readmemh({gen_dir, "/tc_mode2d_sanity_rand_timing_meta.mem"}, meta);
        exp_count = meta[0];
        ...
        if (err == 0) begin
            $display("[PASS] tc_mode2d_sanity_rand_timing");
        end else begin
            $fatal(1, "[FAIL] tc_mode2d_sanity_rand_timing err=%0d", err);
        end
        $finish;
    end
endmodule
