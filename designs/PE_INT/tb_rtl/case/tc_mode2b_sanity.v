...
module tc_mode2b_sanity;
    ...
    initial begin
        ...
        $readmemh({gen_dir, "/tc_mode2b_sanity_tx_a.mem"}, tx_a);
        $readmemh({gen_dir, "/tc_mode2b_sanity_tx_b.mem"}, tx_b);
        $readmemh({gen_dir, "/tc_mode2b_sanity_exp_o0.mem"}, exp_o0);
        $readmemh({gen_dir, "/tc_mode2b_sanity_exp_o1.mem"}, exp_o1);
        ...
        if (err == 0) begin
            ...
            $fatal(1, "[FAIL] tc_mode2b_sanity err=%0d", err);
        end
        $finish;
    end
endmodule
