...
module tc_mode_switch_random;
    ...
    always @(posedge clk) begin
        if (rst_n && vld_out) begin
            if (got_idx >= exp_count) begin
                $display("[ERR][rand] unexpected extra output out0=%0d out1=%0d", $signed(out0), $signed(out1));
                err <= err + 1;
            end else begin
                if (out0 !== exp_o0[got_idx] || out1 !== exp_o1[got_idx]) begin
                    $display("[ERR][rand] idx=%0d got(out0,out1)=(%0d,%0d) exp=(%0d,%0d)",
                        got_idx, $signed(out0), $signed(out1), $signed(exp_o0[got_idx]), $signed(exp_o1[got_idx]));
                    err <= err + 1;
                end
            end
            got_idx <= got_idx + 1;
    ...
    initial begin
        ...
        if (got_idx !== exp_count) begin
            $display("[ERR][rand] expected %0d outputs, got %0d", exp_count, got_idx);
            err = err + 1;
        end
