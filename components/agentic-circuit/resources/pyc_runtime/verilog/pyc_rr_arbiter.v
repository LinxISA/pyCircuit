...
module pyc_rr_arbiter #(
  ...
  always @* begin
    ...
    for (offset = 0; offset < NUM_INPUTS; offset = offset + 1) begin
      ...
      // Two subtracts cover every legal cursor/offset pair without a
      // variable-condition loop.  This keeps the primitive accepted by
      // Yosys, which only permits while-loops in constant functions.
      if (index >= NUM_INPUTS)
