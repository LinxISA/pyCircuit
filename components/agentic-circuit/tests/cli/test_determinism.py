...
class CliDeterminismTest(unittest.TestCase):
    ...
    def test_build_and_run_manifests_are_root_independent(self) -> None:
        with (
            ...
            first_manifest = first_root.joinpath("runs/one/run-manifest.json").read_bytes()
            second_manifest = second_root.joinpath("runs/one/run-manifest.json").read_bytes()
            first_run_result = first_root.joinpath("runs/one/run-result.json").read_bytes()
            second_run_result = second_root.joinpath("runs/one/run-result.json").read_bytes()
